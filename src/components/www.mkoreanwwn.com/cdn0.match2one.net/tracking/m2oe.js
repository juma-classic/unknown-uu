...
(function (window, document) {
  ...
  function firePixel(tagId, segmentKey, params) {
    if (!tagId || !segmentKey) {
      console.warn(
        "m2oe: tag_id or segment_key not provided. Pixel not fired."
      );
      return;
    }
    ...
    switch (command.action) {
      case "init":
        ...
        if (
          ...
          console.warn(
            "m2oe: 'init' command requires an event name and valid tag_id/segment_key parameters."
          );
        }
        break;
      ...
      case "event":
        ...
        if (!eventConfig) {
          console.warn(
            `m2oe: Configuration for event '${eventToTrack}' not found. Event not fired.`
          );
          return;
        }
        ...
        if (eventToTrack === "PageView") {
          ...
          if (eventParams.order_value) {
            pixelSpecificParams.value = eventParams.order_currency_code
              ? `${encodeURIComponent(
                  eventParams.order_currency_code
                )}:${encodeURIComponent(eventParams.order_value)}`
              : encodeURIComponent(eventParams.order_value);
          }
          firePixel(
            eventConfig.tag_id,
            eventConfig.segment_key,
            pixelSpecificParams
          );
        }
        break;
  ...
  var actionQueue =
    window.m2oe && window.m2oe.actionQueue ? window.m2oe.actionQueue : [];
